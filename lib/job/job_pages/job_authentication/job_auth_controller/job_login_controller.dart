...
// ✅ firebase_auth import REMOVED — no longer needed
...
class JobLoginController extends GetxController {
  ...
  Future<void> onLoginSuccess(BuildContext context) async {
    try {
      ...
      if (found) {
        ...
        if (userData?['profile_completed'] == true) {
          storage.write('is_logged_in', true);
          await saveCandidateDataToLocal(userData!);
          Get.offAllNamed("/dashboard");
        } else {
          Get.offAllNamed("/register");
        }
  ...
  /// ✅ Replaces Firebase Auth verifyPhoneNumber.
  /// Just saves the phone number and navigates to the OTP screen.
  ...
  /// ✅ Replaces Firebase Auth credential verification.
  /// Checks if entered OTP matches the last 6 digits of the phone number.
  void verifyOtp(String entered, BuildContext context) async {
    final expectedOtp = phoneNumber.value.length >= 6
        ? phoneNumber.value.substring(phoneNumber.value.length - 6)
        : phoneNumber.value;

    if (entered.trim() == expectedOtp) {
      await onLoginSuccess(context);
    } else {
      Get.snackbar("Invalid OTP", "Please enter the last 6 digits of your phone number.");
    }
  }
}